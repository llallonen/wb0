...
const UnavailableItem = ({ item }) => {
  return (
    <li className="unavailable__item">
      <div className="unavailable-item-col">
        <img className="good__pic" src={item.img} alt="" />
        <div className="good__description">
          <h5 className="good__title">{item.title}</h5>
          <div className="good__info">
            {item.color && <div className="good__color">{item.color}</div>}
            {item.size && <div className="good__size">{item.size}</div>}
          </div>
        </div>
      </div>
      <div className="unavailable-item-col--2">
        ...
        <div className="cart-item__subtotal subtotal subtotal--desktop"></div>
      </div>
    </li>
  );
};
